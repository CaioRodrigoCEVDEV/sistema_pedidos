...
function UserForm({ initial, screens, isAdmin, onClose, onSaved }) {
...
export default function UsersPage() {
  ...
  return <div className="page-stack"><div className="page-heading"><div><p className="eyebrow">ADMINISTRAÇÃO</p><h1>Usuários</h1><p>Gerencie contas, situação e acesso às telas.</p></div><button className="button" disabled={!isAdmin} onClick={() => setEditing(blank)}>Novo usuário</button></div><section className="panel"><div className="search-bar"><input aria-label="Buscar usuários" placeholder="Buscar por nome, e-mail ou código" value={search} onChange={(e) => setSearch(e.target.value)}/><button className="button secondary" onClick={users.reload}>Atualizar</button></div><Resource resource={users}>{() => filtered.length ? <div className="table-scroll"><table className="operations-table"><thead><tr><th>Código</th><th>Usuário</th><th>Administrador</th><th>Situação</th><th></th></tr></thead><tbody>{filtered.map((u) => <tr key={u.usucod}><td>#{u.usucod}</td><td><strong>{u.usunome || "Sem nome"}</strong><small>{u.usuemail}</small></td><td>{u.usuadm === "S" ? "Sim" : "Não"}</td><td><Status tone={u.ususta === "A" ? "success" : "warning"}>{u.ususta === "A" ? "Ativo" : "Inativo"}</Status></td><td><button className="button secondary" disabled={!isAdmin} onClick={() => setEditing(u)}>Editar</button></td></tr>)}</tbody></table></div> : <p className="empty-state">Nenhum usuário encontrado.</p>}</Resource></section>{editing && <UserForm initial={editing} screens={screens.data || []} isAdmin={isAdmin} onClose={() => setEditing(null)} onSaved={() => setRevision((n) => n + 1)}/>}</div>;
}
