...
function groupItems(items) {
  ...
  items.forEach((item) => {
    const tipo = String(item.tipo || "").trim(), modelo = String(item.modelo || "").trim(), marca = String(item.marca || "").trim();
    const key = `${tipo}\u0000${modelo}\u0000${marca}`;
    if (!groups.has(key)) groups.set(key, { tipo, modelo, marca, items: [] });
    groups.get(key).items.push(item);
  });
  return [...groups.values()];
}
function quoteFor(items, total, note) {
  const groups = groupItems(items);
  const headers = new Map();
  groups.forEach((group) => {
    if (!group.tipo || !group.modelo) return;
    const title = `${group.tipo.toUpperCase()} PARA ${group.modelo.toUpperCase()}`;
    headers.set(title, (headers.get(title) || 0) + 1);
  });
  let text = "📦 Orçamento de Peças:\n\n";
  groups.forEach((group) => {
    if (group.tipo && group.modelo) {
      let title = `${group.tipo.toUpperCase()} PARA ${group.modelo.toUpperCase()}`;
      if (headers.get(title) > 1 && group.marca) title += ` - ${group.marca.toUpperCase()}`;
      text += `📱 ${title}\n\n`;
    }
    group.items.forEach((item) => { text += `(${item.qt}) ${item.nome} R$${Number(item.preco).toFixed(2)}\n\n`; });
  });
...
export default function StoreCartPage() {
  ...
  const [note, setNote] = useState(""), [channel, setChannel] = useState("BALCAO"), [busy, setBusy] = useState(false), [error, setError] = useState(null), [priceChecked, setPriceChecked] = useState(false), [loggedIn, setLoggedIn] = useState(false);
  useEffect(() => { seller().then((id) => setLoggedIn(Boolean(id))); }, []);
  useEffect(() => {
  ...
  async function copyQuote() {
    await navigator.clipboard.writeText(quoteFor(cart.items, cart.total, note.trim()));
  }
